...
class FishEditor {
  ...
  insertTextInterceptor(contentText: string, showCursor: boolean, callBack?: (success: boolean) => void): void {
    ...
    if (!maxLength) {
      ...
      // 使用 Array.from 按 Unicode 码点截断，避免从 emoji surrogate pair 中间截断导致乱码
      const sliceByCodePoint = (str: string, len: number) => Array.from(str).slice(0, len).join('')

      // 换行符不算字符数
      if (!isLineBreakCount) {
        const result = contentText.split(/[\r\n]+/).join('')
        const newContentLength = Array.from(result).length
        // truncate
        if (leftLength < newContentLength) {
          insertTextFn(sliceByCodePoint(contentText, leftLength))
          return
        }
        // direct insertion
        insertTextFn(contentText)
        ...
        // truncate
        if (leftLength < Array.from(contentText).length) {
          insertTextFn(sliceByCodePoint(contentText, leftLength))
          return
        }
        // direct insertion
