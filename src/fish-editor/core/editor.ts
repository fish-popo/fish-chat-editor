...
class Editor {
  ...
  public getText(isPure = false, caller?: string) {
    ...
    // 检查缓存是否存在且为最新版本
    if (
      !isEmpty(this.textCache) &&
      this.textCache[cacheKey] &&
      this.textCache[cacheKey].version === this.contentVersion
    ) {
  ...
  public getLength() {
    ...
    // 使用 Array.from 按 Unicode 码点计算长度，避免 emoji 等 surrogate pair 字符被计为2
    return reqText ? Array.from(reqText).length : 0
  }
